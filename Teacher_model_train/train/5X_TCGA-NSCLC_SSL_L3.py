...
def test(lg):
    ...
    lg.test(model,testloader,checkpoint_files,args.csv_path,"l3")
...
def main():
    args.dataset_name="TCGA-NSCLC-ssl-l3"
    logger_lg = logger(dataset_name=args.dataset_name, log_root=args.recoder)
    lg = Lightning(lg_logger=logger_lg,num_class=2)
    for i in range(0,20):
        args.seed = i
        csv_seed, csv_fold = i // 5, i % 5

        args.data = "D:\\Dataset\\TCGA-LUNG\\represention\\all_1042\\stage3"
        args.split_path = "D:\\Dataset\\TCGA-LUNG\\represention\\all_1042\\file_labels_seed{}.xlsx".format(csv_seed)
        args.train_sheet = "train_fold{}".format(csv_fold)
        args.test_sheet = "test_fold{}".format(csv_fold)
        args.csv_path = "./recoder/{}/results/train_on_kflod_TCGA.csv".format(args.dataset_name)
