...
class Attn_Net_Gated(nn.Module):
    ...
    def forward(self, feature, batch, istrain=True):
        ...
        score = score / self.t
        score_softmax = softmax(score, batch)  # [10240,1] []==>[4,10240,1]
...
class Teacher_model(nn.Module):
    ...
    def forward(self, org_x, batch=None, istrain=True):
        if batch is None:
            batch = torch.zeros(size=(len(org_x),), dtype=torch.int64)
        x = self.fc(org_x)

        x_norm, score, score_softmax, patch_norm_feature = self.Attention(x, batch, istrain)
...
def main():
    model = Teacher_model(n_class=4, input_dim=3072, embed_dim=512)
